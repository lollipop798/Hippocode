...
- `/hippo:associate` 的最小只读联想召回执行器
- `/hippo:active-recall` 的最小只读主动召回执行器
...
npm run typecheck
npm run build
npm run cli -- commands --json
npm run validate:memory-schema
npm run smoke
npm run regression:recall
npm run regression:associate
npm run regression:active-recall
...
`npm run regression:associate` 会基于同一组 recall fixture，验证 `/hippo:associate` 的关系扩散召回、graph 关联命中与只读边界。
`npm run regression:active-recall` 会验证 `/hippo:active-recall` 在高风险前置场景下能主动提升 incident / constraint 相关召回，并保持只读。
`npm run regression:project-onboard` 会在隔离的 `.memory` 上验证项目画像初始化，检查 `project-profile`、`current-focus` 和基础 graph 节点是否被正确刷新。
`npm run regression:runtime` 会统一执行 `project-onboard`、`forecast`、`reflect`、`sleep`、`associate`、`active-recall`、`prune`、`status`、`deep-sleep` 的固定回归；对应的单命令入口会读取各自的 `fixtures/*-regression/.memory`，验证项目画像初始化、计划输出、`episodic` 写入、候选层判断、联想召回、主动召回、只读清理建议、状态汇总、长期层晋升与 `nextCommandHint`/`exposureTrace` 合同。
...
`npm run regression:cli` 会基于 `dist/cli/bin.js` 对当前最小 CLI 做固定回归，覆盖 `init`、`validate`、`recall`、`associate`、`active-recall`、`project-onboard`、`forecast`、`reflect`、`sleep`、`prune`、`status` 与 `deep-sleep` 十二个子命令的 JSON 输出合同。
...
`npm run cli -- help` 会运行当前最小 CLI，可直接调用 `init`、`validate`、`recall`、`associate`、`active-recall`、`project-onboard`、`forecast`、`reflect`、`sleep`、`prune`、`status`、`deep-sleep` 十二个子命令。
`npm run cli -- commands --json` 会输出当前已实现的 `/hippo:` 命令描述，便于宿主侧或脚本侧做能力探测。
...
npm run cli -- associate --prompt "runtime recall regression relation expansion" --seed-id incident:I-2026-04-13-001 --depth 1 --exposure focused --json
npm run cli -- active-recall --prompt "before changing runtime recall ranking" --risk-profile high --exposure focused --json
